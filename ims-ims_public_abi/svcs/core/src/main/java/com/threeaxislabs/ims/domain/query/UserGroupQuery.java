...
@GraphQLType
public class UserGroupQuery implements EntityQuery {
    @Filter(path = "id")
    private List<StringParameter> id = emptyList();

    @Filter(path = "roles", index = IndexType.object)
    private List<DocParameter> roles = emptyList();
    ...
    public List<DocParameter> getRoles() {
        return roles;
    }

    public void setRoles(List<DocParameter> roles) {
        this.roles = roles;
    }
