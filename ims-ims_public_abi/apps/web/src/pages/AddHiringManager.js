...
const REGISTER_HiringManager_USER = gql`
  mutation RegisterHiringManagerUser(
    $username: String!
    $emailId: String!
    $password: String!
    $mobileNo: String!
  ) {
    registerAsHiringManagerUser(
      request: {
        username: $username
        emailId: $emailId
        password: $password
...
const AddHiringManager = () => {
    ...
    const [registerUser] = useMutation(REGISTER_HiringManager_USER);
                ...
                <div className="row justify-content-center mt-lg-5">
                    <div className="col-md-4 mt-6" style={{ backgroundColor: '#50E2CC10', borderRadius: '15px', borderInline: '10px', display: 'inline' }}>
                        <form className="registrationForm" onSubmit={handleSubmit}>
                            {/*<h2>Create Account as Company</h2>*/}
                            <div className={`mt-2 mb-3 ${validation.username && !validation.username.isValid ? 'has-error' : ''}`}>
                                <label htmlFor="username">HiringManager Name</label>
                                <input type="text" className="form-control" name="username" placeholder="HiringManager Name" value={username} onChange={(e) => setUsername(e.target.value)} />
...
export default AddHiringManager;
